                   ...
                   unsigned int highest_dma_mode, bool smart_supported)
{
    ...
    print_row("Supported Ultra DMA Modes", highest_dma_mode ? std::to_string(highest_dma_mode) + " and below" : std::to_string(highest_dma_mode), WHITE, COLUMN_WIDTH);
    print_row("SMART Self-Test Support", smart_supported ? "Yes" : "No", smart_supported ? GREEN : RED, COLUMN_WIDTH);
...
{
    std::cout << std::left << std::setw(column_width) << label << ": " << color << value << RESET << "\n";
}
